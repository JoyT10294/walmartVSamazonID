...
def readData(filename_list):
    ...
    for filename in filename_list:
        with open(filename) as f:
            source=filename.split('-10')[0]
            reader = csv.DictReader(f)
            for row in reader:
                clean_row = [source, (k, preProcess(v)) for (k, v) in row.items()]
                row_id = int(row['id'])
...
if __name__ == '__main__':
    ...
    input_files = ['walmart-10.csv', 'amazon-10.csv']
    output_file = 'output_matching_output.csv'
    ...
    if os.path.exists(settings_file):
        ...
        fields = [
            {'field': 'shipweight', 'type': 'Float', 'has missing': True},
            {'field': 'price', 'type': 'Float', 'has missing': True},
            {'field': 'modelno', 'type': 'String', 'has missing': True},
            {'field': 'title', 'type': 'String'},
            {'field': 'brand', 'type': 'String'},
            {'field': 'dimensions', 'type': 'String', 'has missing': True},
            ]
